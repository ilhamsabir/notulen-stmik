...
<div id="content">
  <div id="content-header">
    ...
    <i class="icon-home"></i> Dashboard</a> <a href="<?php if($show->jenis == 'Internal'){ echo base_url('notulis/acara_internal'); } elseif($show->jenis == 'Eksternal') { echo base_url('notulis/acara_eksternal'); } ?>"> List Data Acara</a> <a href="#" class="current">Detail Acara</a> </div>
    <h1>Detail Acara</h1>
  </div>
  <div class="container-fluid">
    ...
    <div class="row-fluid">
      ...
      <div class="span6">
        ...
        <a href="<?php if($show->jenis == 'Internal'){ echo base_url('report/undangan_notulis/'.$show->kode); } elseif($show->jenis == 'Eksternal') { echo base_url('report/undangan_notulisEks/'.$show->kode); } ?>" class="btn btn-danger btn-large" title="Download PDF"><i class="icon-file">
        </i></a>
        <?php if(($show->status)=='') { ?>
        <div class="pull-right">
          <?php if ($show->jenis == 'Internal' && $show->id_pembuat == $this->session->userdata('id')) { ?>
          <a href="<?php echo base_url('notulis/send_invite/'.$show->kode) ?>" class="btn btn-warning btn-large" title="Kirim Undangan"><i class="icon-envelope"></i></a>
          <?php } elseif ($show->jenis == 'Eksternal' && $show->id_pembuat == $this->session->userdata('id')) { ?>
          <a href="<?php echo base_url('notulis/jumlah_inviteEks/'.$show->kode) ?>" class="btn btn-warning btn-large" title="Kirim Undangan"><i class="icon-envelope"></i></a>
          <?php } ?>
        </div>
        <?php } elseif(($show->status)=='Undangan Terkirim' || ($show->status)=='Sedang Berlangsung') { ?>
        <div class="pull-right">
          <?php if ($show->id_pembuat == $this->session->userdata('id')) { ?>
          <a href="<?php echo base_url('notulis/rollback_invite/'.$show->kode) ?>" class="btn btn-default btn-large" title="Rollback Undangan"><i class="icon-envelope"></i></a> 
          <?php } if ($show->id_notulis == $this->session->userdata('id') && $show->jenis == 'Internal') { ?>
          <a href="<?php echo base_url('notulis/create_notulen/'.$show->kode) ?>" class="btn btn-success btn-large" title="Buat Notulen"><i class="icon-book"></i></a>
          <?php } ?>
        </div>
        <?php } elseif($show->status == 'Notulen Dibuat') { ?>
        <div class="pull-right">
          <a href="<?php if($show->jenis == 'Internal') { echo base_url('notulis/detail_notulen/'.$show->kode); } elseif($show->jenis == 'Eksternal') { echo base_url('notulis/detail_notulenEksternal/'.$show->kode); } ?>" class="btn btn-success btn-large" title="Lihat Notulen"><i class="icon-eye-open"></i></a>
        </div>
        <?php } ?>
        </div>
      </div> 
